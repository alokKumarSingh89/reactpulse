...
interface SlowestRequestsProps {
  responses: NetworkResponseEvidence[];
}

export function SlowestRequests({ responses }: SlowestRequestsProps) {
  return (
    <section className="rounded-xl border border-slate-200 bg-white">
      <div className="border-b border-slate-200 p-5">
        <h2 className="font-semibold text-slate-950">Slowest requests</h2>

        <p className="mt-1 text-sm text-slate-500">
          Responses with the longest observed request-to-response duration.
        </p>
      </div>

      {responses.length === 0 ? (
        <div className="p-6 text-sm text-slate-500">
          Request timing evidence was not available.
        </div>
      ) : (
